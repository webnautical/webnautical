...
function Support() {
    ...
    const supportContent = {
      ...
      const supportRecord = async () => {
        try {
          ...
          const response = await fetch(`${config.url}/v1/get_support_details`, {
            method: 'GET',
  ...
  return (
   ...
   <BasicAppBanner content={supportContent}/>
        <section className='text-center glabal_text_page mt-5'>
            <Container>
                <Row>
                    <Col dangerouslySetInnerHTML={{ __html: supportData.content} } className="text-start text-size">
