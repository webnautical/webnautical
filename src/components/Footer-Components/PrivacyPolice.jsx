  ...
  const supportContent = {
    ...
    const supportRecord = async () => {
      try {
        ...
        const response = await fetch(`${config.url}/v1/get_privacy_details`, {
          method: 'GET',
       ...
       console.log("apidata OKKKKKKKKKKKK",apidata)
...
const PolicyCart = ({data}) =>{
  const {heading,suggestText,text1,text2,text3} = data;

  return(
    <div className="glabal_text_page">
      <h2>{heading}</h2>
      <p><strong>Suggested text: </strong>{suggestText}</p>
      {text1&&<p>{text1}</p>}
      {text2&&<p>{text2}</p>}
      {text3&&<p>{text3}</p>}
      <br />
